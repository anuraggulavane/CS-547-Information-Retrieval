...
MY_NAME = "Anurag Gulavane"
MY_ANUM = 901019306  # put your WPI numerical ID here
MY_EMAIL = "agulavane@wpi.edu"

# the COLLABORATORS list contains tuples of 2 items, the name of the helper
# and their contribution to your homework
COLLABORATORS = [("Rahul Chhatbar", "Helped me debug my code")]
...
class Index(object):
    ...
    def index_dir(self, base_path):
        ...
        for filepath in glob.glob(base_path + '**/*.txt', recursive=True):
            with open(filepath, 'r', encoding='utf-8') as file:
                content = file.read()
                self._documents.append(os.path.basename(filepath))
                tokens = self.tokenize(content)
                tokens = self.stemming(tokens)
                for token in tokens:
                    if token not in self._inverted_index:
                        self._inverted_index[token] = []
                    if num_files_indexed not in self._inverted_index[token]:
    ...
    def tokenize(self, text):
        ...
        for char in text:
            if char.isalpha() or char.isdigit():
                ...
                if current_token:
                    token_set.add(''.join(current_token))
                    current_token = []
        if current_token:
            token_set.add(''.join(current_token))
    ...
    def stemming(self, tokens):
        ...
        for token in tokens:
            stemmed = porter.stem(token, 0, len(token) - 1)
    ...
    def boolean_search(self, text):
        ...
        if len(text) == 1:
            if text[0] in self._inverted_index:
                results = [self._documents[i] for i in self._inverted_index[text[0]]]
        elif len(text) == 3:
            if text[1] == 'or':
                results = [self._documents[i] for i in
                           list(set(self._inverted_index[text[0]]).union(set(self._inverted_index[text[2]])))]
            elif text[1] == 'and':
                results = [self._documents[i] for i in
                           list(set(self._inverted_index[text[0]]).intersection(set(self._inverted_index[text[2]])))]
        return results
...
# Commit 54 on 2024-08-22T16:15:57
# Commit 59 on 2024-08-22T16:38:33
# Commit 73 on 2024-08-22T23:51:19
# Commit 9 on 2024-08-22T18:58:44
# Commit 10 on 2024-08-22T18:29:30
# Commit 13 on 2024-08-22T20:55:23
# Commit 14 on 2024-08-22T21:05:32
# Commit 52 on 2024-08-22T22:39:57
# Commit 16 on 2024-11-17T01:12:15
# Commit 43 on 2024-10-04T14:24:08
# Commit 63 on 2024-09-20T21:15:00
# Commit 65 on 2024-12-12T23:49:14
# Commit 68 on 2024-10-13T20:51:56
# Commit 77 on 2024-10-03T18:13:13
